alias clones {
  ...
  while ($line(@~clones,%i)) {
    if ($gettok($line(@~clones,%i),1,32) == %address) clones.addclone %chan %i
      ...
      if (%clones.nicks) {  echo $colour(info) -at 12-(1313clonescan12)-11 $gettok(%clones.nicks,0,32) from %address $+ | unset %clones.nicks }
      set %address $gettok($line(@~clones,%i),1,32)
    }
    inc %i
  }
  if (%clones.nicks) { if ($show) echo $colour(info) -at 12-(1313clonescan12)-11 $gettok(%clones.nicks,0,32) from %address $+  }
